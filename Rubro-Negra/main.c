...
int main() {
    ...
    // Fecha o arquivo e o reabre para leitura dos valores para remoção
    fclose(file);
    ...
    // Lê valores do arquivo e os remove da árvore Rubro-Negra
    while (fscanf(file, "%d", &value) == 1) {
        if (!removeNoRN(arvore, value, informacoes)) {
            fprintf(stderr, "Erro ao remover o valor %d da árvore\n", value);
        }
    }
